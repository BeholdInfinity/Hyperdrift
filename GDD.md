...
| Now | Not yet |
|-----|---------|
| Full-frame docked bay (B1 · B2 · B3; player assigned a random bay each visit); free-look hangar camera (drag pan / scroll zoom); 2.5D industrial set dressing (bay apron tools/terminals only; asymmetric wing lore props; forklift-yard gear at hub ends); three-column pad status boards; danger-lane floor lights | Persistent inventory / loadout across sessions |
...
**Player pad** — assigned to a random bay (B1/B2/B3) each hangar enter. Interim: uses the **same captain checklist** as visitor bays (need meters + red floors + multi-unit requests). Completing the list does **not** launch the ship — the player still owns exit timing. After a short post-service dwell, the player bay waits **10–60s** then re-rolls a new checklist. Real player-request queue (sell / repair / buy / upgrade) remains future Home Base work. Hangar camera defaults on the player ship and supports **click-drag pan** + scroll zoom.
