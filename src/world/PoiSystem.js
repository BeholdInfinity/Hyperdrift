...
export class PoiSystem {
  ...
  _seed() {
    // Home base — known from the start, tracked on ring + map.
    this.register(
      {
        x: STATION.WORLD_X,
        y: STATION.WORLD_Y,
   ...
   * @param {string} source
   */
  register(p, source = POI.SOURCE.MANUAL) {
    const poi = {
      id: `poi${_nextId++}`,
      x: p.x,
      y: p.y,
  ...
  /** Drop a manual waypoint at a world point (auto-tracked). */
  addManualWaypoint(x, y, name = 'Marker') {
    return this.register(
      { x, y, name, iff: 'yellow', discovered: true, onRing: true, onMap: true },
      POI.SOURCE.MANUAL
    );
  }
