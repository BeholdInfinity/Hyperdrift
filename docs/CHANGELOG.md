...
### Changed
- **Kinematic asteroid belts** — ring-band chunks spawn a density-scaled rock count from `sectorLayout.rings[]` (composition + `density`); each rock gets μ-derived circular orbit (`orbitR`, `orbitAngle0`) and live prograde velocity. Open-space random fields cut **50%** (spawn chance + cluster size) and also orbit at their radius from Planet Center. **Fix:** belt placement samples inside the chunk (not random orbit across the whole annulus — that never hit a 2k chunk). **Fix:** belt rocks use a per-chunk orbit catalog materialized by player distance (not spawn-chunk unload) so co-orbit lanes no longer pop out in front of you.
