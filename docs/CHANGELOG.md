...
### Fixed
- **Belt asteroid stream FPS** — restoring full ω·t sector alignment widened the catalog wedge with session time (200+ sectors × dense catalogs per frame) and filled the 150 km retention disk with off-screen kinematic rocks; scan is now center-aligned with canonical sector keys, angular prefilter before `positionAt`, capped sector span with a periodic full pass, and 130 km belt despawn.
- **Belt asteroids empty after flying on** — catalog sector windows capped ω·t at 60 s while live positions used full game time, so after ~1 min (or immediately on long sessions) new spawns landed in the wrong wedge and the belt thinned to empty as you left the initial loaded zone; sector pick now uses the same gameTime as `positionAt()`.
- **Belt asteroids empty after map jumps** — streaming selected catalog sectors by live world angle while catalogs are keyed by t=0 `orbitAngle0`, so as belts sheared the loaded wedge held only far rocks; sector pick now uses the local `orbitAngle0` window (ω·t over playerR ± spawn reach). Dev JUMP HERE runs a short non-staggered catch-up burst (budget-capped).
...
- **Hangar / quick launch egress** — exit spawn heading is **runway egress** (out through the mouth on the runway axis, same for all bays); handoff velocity is station co-orbit **+ 200 u/s REL V along that axis** (`EXIT_REL_SPEED`, negative `alongIngress`); **player exit-burn computer control removed** (full manual control immediately); **3s egress grace** blocks dock and keeps the hull **under the roof draw layer** until it clears geometry (even though REL V exceeds the normal 120 u/s occluder cutoff); zero-hold cleared on handoff; **`_activateSpaceEgress`** shared by quick launch, hangar launch, and combat respawn so playable-space state (camera, input, traffic, expedition) matches once you leave the bay.
