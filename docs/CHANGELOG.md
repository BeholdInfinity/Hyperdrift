...
- **Cockpit MODES corner** — the bottom-right corner (formerly the static **PREC** readout) is now a stack of clickable two-position switches titled **MODES**:
  ...
  - **VIEW** — a viewport-mode switch (`SHIP` / `SCAN`), toggled by the switch or the **V** key. **SHIP** (default) shows the world through the viewport circle with the thin scanner ring around it (the original behavior); **SCAN** replaces both with **one full-disc radar scope** — the ship sits at center, contacts plot by range across the whole radius (nothing collapses to the inner border), and range rings, compass ticks, sweep, nose/tail heading lines, and velocity chevrons scale to fill the disc. The outer POI rim ring is unchanged. In SCAN the disc is a scope, not the world, so pointer weapon aim/fire is suppressed there and clicks anywhere on the disc pick the nearest blip (`GameEngine.scanView` / `toggleScanView`; shared scope geometry via `_scannerGeometry`, driving `ScannerSystem`/`Scanner` through a `fullScope` flag + `plotPad`). Switching between `PORT` and `SCAN` drops the radar-stepped position cache so every blip **snaps to the new view's ring/scope layout immediately** instead of hanging at the old coordinates until the next sweep ping (`ScannerSystem` geometry-key check).
