...
src/
  ...
  systems/
    ...
    ScanVisual.js           FLS FX (seek beams, acquire lerp, silhouette pair-chords + rim beams) + CONTACT preview helpers
...
| Title screen (bokeh-blurred space sim + *Stranger* wordmark/ship; ENTER HANGAR / QUICK LAUNCH / SETTINGS / BLUEPRINT; version stamp; **planned:** last-docked backdrop + last-piloted ship — `GDD.md`) | Done (personalization planned) |
...
### Polish / follow-ups
- **Contact Occlusion and Cockpit UX (shipped v0.1.292–294, opt-in)** — shared `ContactOcclusion.js` for radar / forward scanner / viewport shadows / SCAN darkening; input remap (Mouse back = grapple, Mouse forward = hold FLS, MMB = selection only); FLS SEEK/ACQUIRE/SCAN/RELEASE FX (SCAN = silhouette pair-chords) + CONTACT previews (viewport-matched rotation, silhouette-clipped) + CONTACTS scan row; asteroid scan 1–15 s by tier; COMMS excludes rocks/ore; STATUS tabs + Grapple Arm row; ore radar OTHER contacts; full asteroid pip range. Hangar Bay Scanners use the same pair-chord FX during board reveal / final scan; FLS does not scan stations. **Off by default** (Settings → Display → Contact occlusion; when off, no occluder logic runs); dev drawer → **Occlusion** submenu tunes caster/occluder counts + samples (Save bakes `Constants.js`); shadows use a separate 64-caster cap; angular prefilter skips non-overlapping pairs. Decisions in workspace plan `contact_occlusion_and_cockpit_ux_a9b4a670.plan.md`.
- **FLS Mk-tier scan model (planned — not built)** — scanner **Mk** sets pair count (1 / 1 / 2 / 2 / 3) and base/max beam speed + distance; **pips** (≤ Mk) lerp base→max; **9 scan passes** to 100%; constant perimeter travel speed so larger silhouettes take longer naturally. Replaces hardcoded 1–15 s / type-duration tables. Design in [`GDD.md`](GDD.md) Orientation → FLS Mk tiers; open numbers in [`OPEN_QUESTIONS.md`](OPEN_QUESTIONS.md) §9.5.
...
- **v0.1.294** FLS SCAN silhouette pair-chords (rim open/close + CONTACT silhouette clip); hangar Bay Scanners briefly removed (restored under Unreleased with pair FX; station FLS dropped instead)
- **v0.1.293** FLS hangar-style SEEK/ACQUIRE/SCAN/RELEASE FX + CONTACT/CONTACTS/COMMS scan polish; asteroid scan 1–15 s by tier; belt/HUD/particle frame-rate pass (radar range prefilter, module/nebula/moon caches, batched particles)
