...
- **Input remap (v0.1.292) + FLS look/feel (v0.1.293+)** — side buttons captured with `preventDefault` (no browser back/forward): **Mouse back (M3)** = grapple click in viewport, **Mouse forward (M4)** hold = **FLS** forward scanner, **MMB** = selection only (no grapple). **FLS** passively scans contact types in a **±45° nose cone** while held in SHIP view with scanner pips > 0: **SEEK** (desynced nose beams) → **ACQUIRE** (beams converge on target) → **SCAN** (silhouette pair-chords opening opposite ways around the rim + nose beams to rim points) → **RELEASE** back to SEEK; fully scanned contacts are ignored for further focus. **Today (interim):** three pairs always; asteroid scan **1–15 s** by size tier; ore / ship / station use type base × size scale; range from scanner pips (560 u → 20 000 u at Mk5). **Planned:** Mk-driven pair count + perimeter-speed scan passes — see **FLS Mk tiers (planned)** below. Auto-selects closest unscanned in cone. Scan state is session-scoped per contact; dead refs prune. CONTACT panel shows a type-appropriate preview with pair-chords only while that contact is actively being scanned (clipped to the drawn silhouette); CARGO row unlocks at 100% scan (asteroids/ore). **CONTACTS** list keeps scan extras on one row (drops/cargo · `%` / green **100%** left of range). **COMMS** live channel is ships/stations only (asteroids and ore show NO COMMS TARGET).
...
- **Mining** — laser targets one module; heat cracks build then pop (time scales by material + laser Mk). Popped module only — rest of asteroid stays. Ore drops inherit parent velocity with a slight random outward drift; ship gravity + proximity pickup; **Mouse back (M3)** grapple in viewport (ship-relative shot from the belly, cable under hull, auto-reel on grab or max cable). Ore tallies in **COMMS → Message Log** (`[COMPUTER]` stub until cargo grid). **Shipped (v0.1.292):** Mouse forward (M4) hold = forward scanner (FLS) on all contact types; shared contact occlusion across radar/FLS/viewport/SCAN; ore on radar OTHER chip at full pip range.
...
- [x] **Contact Occlusion and Cockpit UX** — shared LOS, FLS, input remap, STATUS tabs, ore radar (v0.1.292); FLS hangar-style FX + scan UX polish (v0.1.293); FLS silhouette pair-chords + hangar bay scanners removed (v0.1.294)
