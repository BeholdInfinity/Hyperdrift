...
**Flight model:** one **`planet.gravityMu`** drives player gravity (`GravitySystem`) and all circular kinematic orbits (stations, gates; belt rocks use the same ω = √(μ/R³)). Thera gravity is **full inverse-square everywhere in the overworld** (no influence-radius fade). **Deep instance** flights (`enterDeepInstance` / warp-instance gates) set `affectedByGravity = false` until return. Player ship integrates thrust + gravity; circular bodies use analytic Keplerian motion from the same μ. No global hard speed cap — TELEMETRY **PRO** shows circular prograde at your radius when Thera gravity applies; **SYNC** (`X` hold at ≥95%) trims to a selected contact's live velocity. Posted **LIM** is regulatory (fines when caught), not a physics clamp — **local station shells** measure excess speed **relative to that station's orbit** (same frame as docking: match orbit, then bleed down for the mouth). Each station's runway points **upstream** (opposite prograde); hangar / quick-launch egress inherits station co-orbit plus **200 u/s REL V out through the mouth** on the runway axis (full manual control; **3 s egress grace** blocks dock; zero-hold cleared on handoff). Six **ring warp gates** teleport paired hops through the planet hub.
...
- **Bay service display** (crew): three-column pad status board south of each pad (**fixed face height**; northern lip does not move). Left: ship stats with display-only Mk labels. Middle: cargo hold grid by ship size (or `NO CARGO BAY`) — **1×1 slots only** (one load/unload = one slot); filled cells are top-down crate lids matching hangar 2.5D freight colors/textures (load/unload keep the same crate). Right: service checklist + header light; footer shows bay id. After pad settle the board **reveals in stages**: SHIP STATS rows top→bottom (~2s) → CARGO panel (~1s) → brief hold → then within ≤2s the captain “orders” service pips one at a time (random pick order; 0.2–0.6s between different types, 0.1–0.2s when repeating the same type) before deck work starts. Checklist lists each job type once (`Hull` / `Fuel` / `Bullet` / `Shells` / `Install` / `Load` / `Unload`) with a fixed label column (width locked to `Install:`) and a shared **9-slot** pip grid — the labeled row shows up to **5** pips; overflow wraps to **continuation rows** (pips only, no repeated label) with up to **9** pips each. Rows **right-align** within their grid band; each pip keeps a **fixed slot** in that job’s layout (reveal and color changes never move it). Long lists **scroll inside the service column** (completed rows rise to the top; auto-scroll stays pinned to the top until the first job finishes, then scrolls past that job and settles on the highest position that shows the most pending rows; right-edge scrollbar + wheel-over-column manual scroll pauses auto for ~4s). Each **Install** pip is a distinct backend request (target hardpoint + catalog item); the board still shows a single Install row. Forklift brings that part; mechanics strip only that hardpoint if occupied before installing. Circle colors: yellow until a bay mechanic claims that unit; blue while they work it; green only after they walk away from finishing it; grey gated. When every pip for a job type is green, a checkmark appears beside the **labeled row only** (continuation rows never get a gutter check). Header light off / yellow / green / flash red for bay service mood.
...
- **Input remap (v0.1.292) + FLS look/feel (v0.1.293+)** — side buttons captured with `preventDefault` (no browser back/forward): **Mouse back (M3)** = grapple click in viewport, **Mouse forward (M4)** hold = **FLS** forward scanner, **MMB** = selection only (no grapple). **FLS** passively scans **ships, asteroids, and ore** in a **±45° nose cone** while held in SHIP view with scanner pips > 0 (**stations are not FLS targets** — radar / COMMS / dock only): **SEEK** (desynced nose beams) → **ACQUIRE** (beams converge on target) → **SCAN** (silhouette pair-chords opening opposite ways around the rim + nose beams to rim points) → **RELEASE** back to SEEK; fully scanned contacts are ignored for further focus. **Today (interim):** three pairs always; asteroid scan **1–15 s** by size tier; ore / ship use type base × size scale; range from scanner pips (560 u → 20 000 u at Mk5). **Planned:** Mk-driven pair count + perimeter-speed scan passes — see **FLS Mk tiers (planned)** below. Auto-selects closest unscanned in cone. Scan state is session-scoped per contact; dead refs prune. CONTACT panel shows a type-appropriate preview with pair-chords only while that contact is actively being scanned (clipped to the drawn silhouette); CARGO row unlocks at 100% scan (asteroids/ore). **CONTACTS** list keeps scan extras on one row (drops/cargo · `%` / green **100%** left of range). **COMMS** live channel is ships/stations only (asteroids and ore show NO COMMS TARGET). Hangar board **Bay Scanners** use the same silhouette pair-chord FX while revealing / final-scanning a pad ship.
- **FLS Mk tiers (planned — not built)** — replace hardcoded scan timers with silhouette travel + scan passes. **Mk** (hardware) sets pair count and base/max beam curves; **pips** (power, capped by Mk) interpolate between that Mk’s base and max for **beam speed** and **beam distance** (cone range). Pair count does **not** come from pips.
  - **Scan pass:** one silhouette chord-pair completing a full open→meet-opposite→close cycle. An object needs **9 scan passes** for 100% (~11.1% each). Parallel pairs accumulate passes independently (more pairs → faster full scan on the same object).
  - **Perimeter speed:** beams travel the silhouette at a **constant world speed** (the live beam speed from Mk/pips), independent of object size. Larger silhouettes take longer per pass by geometry alone — no separate size timer table.
  - **Mk table** (v1…v5 = tunable speed/distance steps; numeric values TBD — [`OPEN_QUESTIONS.md`](OPEN_QUESTIONS.md) §9.5):

    | Mk | Pairs | Base speed | Max speed | Base distance | Max distance | Pip max |
    |----|-------|------------|-----------|---------------|--------------|---------|
    | 1 | 1 | v1 | v1 (= base) | v1 | v1 (= base) | 1 |
    | 2 | 1 | v2 | v2 | v2 | v2 | 2 |
    | 3 | 2 | v2 | v3 | v2 | v3 | 3 |
    | 4 | 2 | v3 | v4 | v3 | v4 | 4 |
    | 5 | 3 | v3 | v5 | v3 | v5 | 5 |

  - **Implications:** Mk1 with 1 pair is slowest throughput; Mk5 with 3 pairs finishes the same rock faster. Small rocks finish sooner than large ones at the same Mk/pips. SEEK/ACQUIRE/RELEASE should use the same live pair count as SCAN.
- **Contact occlusion (v0.1.292, opt-in)** — shared line-of-sight (`ContactOcclusion.js`): circle prefilter then polygon (asteroids/ships) / AABB (stations) / circle (ore) tests; nearest-24 occluder cap for visibility, separate 64-caster shadow cap; angular prefilter skips contacts whose bearing never overlaps the target. Consumers: radar blip gate (fully blocked → no blip; partial → dim), FLS range cap, **SHIP** viewport pitch-black umbra polygons (drawn before the player ship, so the grapple cable — belly origin — stays under the hull), **SCAN** disc darkened wedges. Occluded selected contact keeps last-known RANGE with a **SIGNAL BLOCKED** badge and drops after 5 s. **Off by default** — Settings → Display → Contact occlusion; when off, no occluder logic runs (visibility = 1 for all contacts). Dev drawer → **Occlusion** tunes caster/occluder counts + samples, Save bakes `Constants.js`.
...
- **Mining** — laser targets one module; heat cracks build then pop (time scales by material + laser Mk). Popped module only — rest of asteroid stays. Ore drops inherit parent velocity with a slight random outward drift; ship gravity + proximity pickup; **Mouse back (M3)** grapple in viewport (ship-relative shot from the belly, cable under hull, auto-reel on grab or max cable). Ore tallies in **COMMS → Message Log** (`[COMPUTER]` stub until cargo grid). **Shipped (v0.1.292):** Mouse forward (M4) hold = forward scanner (FLS) on ships / asteroids / ore (not stations); shared contact occlusion across radar/FLS/viewport/SCAN; ore on radar OTHER chip at full pip range.
...
Sparse NPC ships in open space (modular catalog defs). **Multi-station:** spawns anchor on live orbital ports (weighted by `patrolDensity`; pirate hub `trafficPolicy: none` skipped for local spawns). **Social-tier radial affinity:** ship class pools keyed to `socialOrbitInner` tier lines — peak spawn probability at each tier’s orbit radius with low-but-nonzero cross-tier spillover (`SocialTierTraffic.js`). Per-station density still falls off with distance from the anchor. Deep space almost empty but not zero. **Police are always present** system-wide (**exactly 3**: 1× Heavy fighter Mk5 + 2× Standard fighters Mk2 — anchored on the **military-tier** station; police theme; hex ring at **station edge + 2× runway length**; scan). AI arrivals use the same **Alt Space-brakes** (retro when nose faces into velocity). Behaviours by class (police, flyby, race, mine, survey, shuttle/freight, heavy deep cruise). **All ambient turns and burns are thruster/engine Newtonian** via shared `NpcPilot` (straight + at cruise → thrusters off / coast facing **prograde**; burn only when nose is on the burn axis; brake for arrival; only non-Newtonian soft cap is max speed — no angle/velocity snaps). Thruster/engine **plumes + ship-local exhaust particles** use the same mount-driven path as the player (`PlumeDraw`). Police and lane freight use **hexagon waypoint rings**; station-full holds use the north-of-runway racetrack. **Spawn and despawn only off-screen** (outside the circular play viewport + margin) so ships never pop in or out of view; age expiry while visible steers them off-camera first. Hard caps + off-screen cull keep performance stable. **Bay mouth traffic:** while the pilot is in space, the live hangar requests door fills as **customers from a wide ring around the station** (any bearing — not only from the north). They fly inbound to a north-runway staging point, then final approach / hold; departures push out as egress burns — paced to match hangar-inside visitor cadence. Holders leave the racetrack immediately when a visitor pad opens. **Tuning note (2026-07-25):** exterior **`HangarPresence`** bridge + reservation spawns are first-pass only — co-orbit inbound catch-up, elevator-only exterior beacons, and long-session cadence still need another tuning pass (see `PROJECT.md` Known gaps).
...
The title screen is the player's **resume surface**. The game **save point** is the player's **last docked station** (orbital port). Title presentation is planned to follow that anchor:

| Element | Plan |
|---------|------|
| **Backdrop** | Live bokeh-blurred space sim at the **last docked location** — the same port the player would resume at after save/load (ambient traffic, asteroids, runway activity; camera bob + framing tuned per station) |
| **Showcase ship** | The **last ship the player had pilot control over** (pilot seat), rendered in 2.5D **behind** the *Stranger in the Galaxy* wordmark |

**First launch / no save yet:** default to Jennings Station backdrop + starter Generalist ship until the player docks somewhere and that session persists.

**Today (prototype):** backdrop is fixed to Jennings Station; showcase ship is the dedicated default Generalist (`src/ui/TitleScreen.js`), not yet read from save state. Combat respawn already uses last-docked station (Jennings fallback); title personalization ships with full save/load.
...
- [x] **Contact Occlusion and Cockpit UX** — shared LOS, FLS, input remap, STATUS tabs, ore radar (v0.1.292); FLS hangar-style FX + scan UX polish (v0.1.293); FLS silhouette pair-chords (v0.1.294); hangar Bay Scanners restored with pair-chord FX + FLS skips stations (Unreleased)
- [ ] **FLS Mk-tier scan model** — pair count by scanner Mk; 9 scan passes to 100%; constant perimeter beam speed (size from geometry); pips lerp base→max speed/distance within Mk (see Orientation → FLS Mk tiers)
- [ ] Missions / quests
...
- [x] Minimap / radar implementation — **SECTOR MAP** panel + **VIEW → SCAN** full scope (no corner minimap)
