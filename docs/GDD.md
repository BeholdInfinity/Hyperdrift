...
- **Input remap (v0.1.292)** — side buttons captured with `preventDefault` (no browser back/forward): **Mouse back (M3)** = grapple click in viewport, **Mouse forward (M4)** hold = **FLS** forward scanner, **MMB** = selection only (no grapple). **FLS** passively scans all contact types in the nose cone (mining-laser arc) while held in SHIP view with scanner pips > 0; per-type durations asteroid **3 s** / ore **2 s** / ship + station **10 s**; tier-1 range 280 u scaling linearly to 20 000 u at Mk5; auto-selects closest in cone (overrides manual while scanning). Scan state is session-scoped per contact; dead refs prune. CARGO row on CONTACT unlocks at 100% scan (asteroids/ore).
