...
- **Input remap (v0.1.292) + FLS look/feel (v0.1.293+)** — side buttons captured with `preventDefault` (no browser back/forward): **Mouse back (M3)** = grapple click in viewport, **Mouse forward (M4)** hold = **FLS** forward scanner, **MMB** = selection only (no grapple). **FLS** passively scans contact types in a **±45° nose cone** while held in SHIP view with scanner pips > 0: **SEEK** (6 desynced nose beams) → **ACQUIRE** (beams converge on target) → **SCAN** (three silhouette pair-chords opening opposite ways around the rim + nose beams to the six rim points) → **RELEASE** back to SEEK; fully scanned contacts are ignored for further focus; asteroid scan **1–15 s** across all size tiers (proc + hero); ore / ship / station use type base × size scale; tier-1 range 560 u scaling to 20 000 u at Mk5; auto-selects closest unscanned in cone. Scan state is session-scoped per contact; dead refs prune. CONTACT panel shows a type-appropriate preview with pair-chords only while that contact is actively being scanned; CARGO row unlocks at 100% scan (asteroids/ore). **CONTACTS** list keeps scan extras on one row (drops/cargo · `%` / green **100%** left of range). **COMMS** live channel is ships/stations only (asteroids and ore show NO COMMS TARGET).
...
- [x] **Contact Occlusion and Cockpit UX** — shared LOS, FLS, input remap, STATUS tabs, ore radar (v0.1.292); FLS hangar-style FX + scan UX polish (v0.1.293); FLS silhouette pair-chords + hangar bay scanners removed
- [ ] Missions / quests
